...
static bool ensure_vendor_module_is_loaded(void)
{
    ...
    if (!vendor.module) {
        int rv = load("fingerprint", HAL_VENDOR_PATH"/fingerprint.default.so", &vendor.hw_module);
        if (rv) {
...
static int set_notify(struct fingerprint_device *dev, fingerprint_notify_t notify)
{
    ...
    return device->vendor.device->set_notify(device->vendor.device, hal_notify_convert);
}
